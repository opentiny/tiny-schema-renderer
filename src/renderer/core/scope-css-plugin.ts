/** @ref {@vue/compiler-sfc@3.5.27/src/style/pluginScoped.ts } */
/* eslint-disable prefer-const*/
import { type AtRule, type Container, type Document, type PluginCreator, Rule } from 'postcss';
...
function processRule(id: string, rule: Rule) {
  if (
    ...
    (rule.parent && rule.parent.type === 'atrule' && keyframesRE.test((rule.parent as AtRule).name))
  ) {
    return;
  }
...
function rewriteSelector(
  ...
  selector.each((n) => {
    ...
    if (
      ...
      (n.type === 'pseudo' && (n.value === ':is' || n.value === ':where') && !node)
    ) {
      node = n;
    }
  });
